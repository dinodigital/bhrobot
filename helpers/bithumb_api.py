...
class BithumbGlobalRestAPI:
    ...
    def place_order(self, symbol, type, side, price, amount):
    ...
    def place_order_2(self, params):
        symbol = params['symbol'].replace('/', '-')
        parms = {
            'symbol': symbol,
            'type': params['type'],
            'side': direction(params['side']),
            'price': '%.8f' % params['price'],
            'quantity': '%.8f' % params['quantity']
        }
        return self.post('placeOrder', parms)['orderId']

    def get_current_price(self, symbol, side):
        order_book = self.post('orderBook', {'symbol': symbol})

        if side == "buy":
            return order_book['b'][0][0]
        else:
            return order_book['s'][0][0]
    ...
    def wallet_balance(self, coin=None):
        parms = {
            'assetType': 'wallet'
        }
        if coin:
    ...
    def orders(self, side, symbol, status="trading", queryRange='thisweek', page=1, count=10):
        parms = {
            'page': str(page),
            'count': str(count),
